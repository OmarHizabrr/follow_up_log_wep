      ...
      <aside className="fixed inset-y-0 right-0 hidden lg:flex flex-col w-72 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 z-50 shadow-lg ring-1 ring-black/5 transition-colors duration-200">
        ...
        <div className="h-[72px] flex items-center px-6 border-b border-gray-200 dark:border-gray-700 shrink-0">
          <div className="flex items-center gap-3">
             <div className="w-9 h-9 rounded bg-blue-600 flex items-center justify-center p-1.5 shadow-sm">
               <img src="/images/logo/logo.png" alt="Logo" className="w-full h-full object-contain filter brightness-0 invert" />
             </div>
             <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">نظام المتابعة</h1>
          </div>
        </div>

        {/* Navigation */}
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-1.5">
          <div className="text-[11px] font-bold text-gray-400 dark:text-gray-500 mb-4 px-3 uppercase tracking-wider">
             القائمة الرئيسية
          </div>
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href}>
                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                  isActive 
                    ? 'bg-blue-50 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 font-bold shadow-sm' 
                    : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50'
                }`}>
                  <Icon className={`w-5 h-5 transition-colors ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`} />
      ...
      <div className="lg:pr-72 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-40 h-[72px] bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6 lg:px-8 shrink-0 transition-colors duration-200 shadow-sm">
           ...
           <div className="flex items-center gap-4 shrink-0">
              ...
              <div className="hidden sm:flex items-center gap-3 cursor-pointer p-1 pr-3 rounded-full hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700">
                 <div className="text-left hidden lg:block">
                   <p className="text-sm font-bold text-gray-900 dark:text-white">{user?.displayName?.split(' ')[0] || 'مستخدم'}</p>
                 </div>
                 <div className="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-sm shadow-sm ring-2 ring-white dark:ring-gray-800">
                   {user?.displayName ? user.displayName[0] : 'U'}
                 </div>
              </div>
           </div>
